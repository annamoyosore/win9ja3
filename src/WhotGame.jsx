...
function createDeck() {
  let d = [];
  Object.entries(VALID).forEach(([s, nums]) => {
    nums.forEach(n => d.push(s + n));
  });
...
function isValid(c) {
  if (!c) return false;
  return VALID[c[0]]?.includes(Number(c.slice(1)));
}
...
function drawCard(card) {
  ...
  if (card.shape==="circle"){ctx.beginPath();ctx.arc(cx,cy,10,0,Math.PI*2);ctx.fill();}
...
function drawBack(){
  ...
  ctx.fillStyle="#111"; ctx.fillRect(0,0,65,100);
  ctx.strokeStyle="#fff"; ctx.strokeRect(2,2,61,96);
...
function parseGame(g){
  const handsRaw = g.hands?.split("|").map(p=>p.split(",").filter(Boolean))||[[],[]];

  return {
    ...g,
    players: g.players?.split(",")||[],
    deck: g.deck?.split(",").filter(isValid)||[],
    hands: handsRaw.map(h=>h.filter(isValid)),
    history: g.history?.split("||")||[],
...
const styles={
  ...
  history:{marginTop:10,maxHeight:120,overflow:"auto",fontSize:12},
